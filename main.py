        ...
        for job_index in range(num_jobs):
            for t_0_based in range(num_time_slots):
                if t_0_based + 1 >= t_hat:
                    ...
                    assigned_job_id = partial_schedule.get(t_0_based + 1, None)
...
def schedule_jobs(jobs):
    ...
    for current_timeslot in range(1, jobs.total_time_slots + 1):
        ...
        # deal with the last time slot will be tricky because there is no next time slot to compute bounds for, so we just pick the job with the highest lower bound
